...
func NodeCreate(operator *types.User, nodeCh chan types.NodeSimpleInfo, nodes models.NodesCreateReqInfo) ([]*types.Node, error) {
    nodeList, err := Nodes()
    if err != nil {
        return nil, err
    }
    ...
    for _, node := range addNodes {
        for _, existNode := range nodeList {
            if node.IpAddress == existNode.IpAddress {
                return nil, response.NewBadRequestErr(locales.CodeNodesIpAddressAlreadyExist)
            }
        }
    }
